...
for inst in range(10):
    np.random.seed(1789 + 31*inst)
    rand_list = np.random.random(3)
    ...
    constants_dict['fyd'] = (0.9 + 0.2*rand_list[0])*constants_dict['fyd']
    constants_dict['hxd'] = (0.9 + 0.2*rand_list[0])*constants_dict['hxd']
    constants_dict['txd'] = (0.9 + 0.2*rand_list[0])*constants_dict['txd']
...
#    constants_dict['yd']  = (0.9 + 0.2*rand_list[12])*constants_dict['yd']  # ShankInertia
    ...
    for ll in range(id_sta, id_end):
        ...
        p_init = imped_sim*(2*np.random.random(len(imped_sim)))
