...
if num_phase == 2:
    ...
    impedance = np.array([1.5, 0.005, 2.0, 0.02, 0.1, 0.01, 0.1, 0.02, 
                              2.5, 0.012, 5, 0.01, 0.5, 0.017, 0.5, 0.014])*mass   # ankle and knee joints phase 4
    
impedance_sim = np.loadtxt('../Simulation_Study_Data/Simulation_Id_New/Simulated_imped.txt')
motion_sim = np.loadtxt('../Simulation_Study_Data/Simulation_Id_New/Simulated_motion.txt')
torque_per_sim = np.loadtxt('../Simulation_Study_Data/Simulation_Id_New/Simulated_torques_pero.txt')
torque_sim = np.loadtxt('../Simulation_Study_Data/Simulation_Id_New/Simulated_torques.txt')
...
motion_meas[:, 0] = motion_meas[:, 0] + np.linspace(0, num_nodes*interval, num_nodes)*(0.8 + 0.4*T)
...
#
#color = ['k.', 'b.', 'g.', 'r.']
#for i in range(len(motion_meas[:, 0])):
#    ax1.plot(i, motion_meas[i, 3], color[int(index2_imped[i, 0]-1)])
#    ax1.plot(i, motion_meas[i, 4], color[int(index2_imped[i, 0]-1)])
#    ax1.plot(i, motion_meas[i, 5], color[int(index2_imped[i, 0]-1)])
#    
#    ax2.plot(i, motion_meas[i, 6], color[int(index2_imped[i, 1]-1)])
#    ax2.plot(i, motion_meas[i, 7], color[int(index2_imped[i, 1]-1)])
#    ax2.plot(i, motion_meas[i, 8], color[int(index2_imped[i, 1]-1)])
#    
...
#    
#    ax5.plot(i, moment_meas[i, 0], color[int(index2_imped[i, 0]-1)])
#    ax5.plot(i, moment_meas[i, 1], color[int(index2_imped[i, 0]-1)])
#    ax5.plot(i, moment_meas[i, 2], color[int(index2_imped[i, 0]-1)])
#    
#    ax6.plot(i, moment_meas[i, 3], color[int(index2_imped[i, 1]-1)])
#    ax6.plot(i, moment_meas[i, 4], color[int(index2_imped[i, 1]-1)])
#    ax6.plot(i, moment_meas[i, 5], color[int(index2_imped[i, 1]-1)])
...
mom_c_meas_vec = torque_sim[:, index_open].T.flatten('F')
mom_c_norm_vec = torque_per_sim[list(np.linspace(0, 100, num_normal, dtype=int)), :].T.flatten('F')
...
#plt.plot(motion_meas[:, [4 ,5]], '--')
#
#plt.figure()
#plt.plot(x_normal_ref[:, [2, 3]])
#plt.plot(motion_meas[:, [13 ,14]], '--')
...
store_path = ('../Simulation_Study_Data/Simulation_2dData_Wmt'+str(weight_mt)+'Wms'+str(weight_ms)+'Wts'
              ...
              +gender+str(S)+'_S'+str(T)+'_N'+str(num_nodes)+'_JC'+ str(num_control) 
              +'_Phase' + str(num_phase) + 'GoodInit2/')
...
for ll in range(id_sta, id_end):
    ...
    p_init = impedance_sim*(0.8+0.2*np.random.random(len(impedance_sim)))
