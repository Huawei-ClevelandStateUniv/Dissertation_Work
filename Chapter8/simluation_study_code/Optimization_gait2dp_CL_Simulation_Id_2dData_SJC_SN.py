...
for inst in range(10):
    ...
    pink_noise = generate_pink_noise(num_joints, num_nodes, rand_seed)
...
#    constants_dict['yd']  = (0.9 + 0.2*rand_list[12])*constants_dict['yd']  # ShankInertia
    ...
    store_path = ('../Simulation_Study_Data/Simulation_Id_SN_'+str(inst)+'/')
    ...
    for ll in range(id_sta, id_end):
        ...
        p_init = imped_sim*(0.25 + 2.75*np.random.random(len(imped_sim)))
