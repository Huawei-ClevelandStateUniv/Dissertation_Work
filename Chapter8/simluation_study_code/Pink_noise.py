...
def generate_pink_noise(num_joints, num_nodes, ran_seed):
    
    T_sim_filt = np.zeros((num_nodes, num_joints))
    ...
    for k in range(num_joints):
        ...
        white_noise_sigma =  2e-5 #mK * sqrt(s) Noise power for hip joint
        
        SFREQ = 100 #Hz
        KNEE = 10 #Hz
        ALPHA = 1
        N = num_nodes # 10 seconds
        
        #generate white noise in time domain
        np.random.seed(1789+31*ran_seed + 3*k)
    ...
    #    plt.grid(); plt.xlabel('Freq'); plt.title('Amplitude spectrum'); plt.legend()
        ...
        b, a = signal.butter(1, 6/(50/2))
        
        T_sim_filt[1:, k] = signal.filtfilt(b, a, T_sim)
