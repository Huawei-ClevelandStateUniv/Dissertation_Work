...
index_l_hs = remove_outlayer(np.where(np.diff(index_l) == 1)[0]) -4
index_r_hs = remove_outlayer(np.where(np.diff(index_r) == 1)[0]) -4
...
average_l_grf = np.zeros((101, 3))
average_r_grf = np.zeros((101, 3))
...
for n in range(len(index_r_hs)-1):
    ...
    for l in range(18):
        ...
        if l < 3:
            average_r_grf[:, l] += np.interp(time_average, time_real_r, grf1[index_r_hs[n]:index_r_hs[n+1], 3+l])
...
if j < 4:
    store_path_ave = 'averaged_normal_gaits_1/F'+str(j)+'_S'+str(s)
else:
    store_path_ave = 'averaged_normal_gaits_1/M'+str(j-4)+'_S'+str(s)
