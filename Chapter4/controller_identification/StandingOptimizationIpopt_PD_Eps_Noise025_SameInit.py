...
import ipopt
import numpy as np
from StandingModel_PD_Eps_Noise_SameInit import Model
...
num_par = 6  # number of parameters of the identification controller 
...
lb = np.hstack((lb_x, lb_c))
ub = np.hstack((ub_x, ub_c))
...
Dictionary1 = 'Results_150_250/Subj0' +str(subj)+ '/Pert1_GFL_PD_Eps'+str(num_repeat)+'_Noise025_SameInit/'
...
# generate experimental data for the multiple episodes identification
x_exp1 = np.array([])
acc_exp1 = np.array([])
...
def do_optimization(j):
    ...
    np.random.seed()
    noise1 = NoiseAmp/Scaling - 2.0*NoiseAmp/Scaling*np.random.random((num_nodes*num_repeat, int(num_states/2)))
    ...
    nlp1.addOption(b'max_iter', 2000)
    nlp1.addOption(b'tol', 1e-6)
    nlp1.addOption(b'acceptable_tol', 1e-4)
    nlp1.addOption(b'max_cpu_time', 1e+4)
    
    # randomize the initial guess of the control parameters
    unknown_p1 = np.zeros(num_par)
    np.random.seed()
    unknown_p1[:num_par-2] = 5000.0/Scaling*np.random.random(4)
    np.random.seed()
    unknown_p1[num_par-2:] = 0.5 - 1*np.random.random(2)
...
def mp_handler():
    num_parallel = 4  # number of parallel computing in CPUs
