...
This code is to define the optimization functions for proportional-derivative (PD)
...
class Model(object):
    ...
    '''
    def __init__(self, x_meas, accel_meas, num_eposides, num_nodes, num_states, num_par,
...
#        grad[:self.num_states*self.num_nodes] = 2.0*self.interval*(x[:self.num_states*self.num_nodes] - self.x_meas)
    ...
    def constraints(self, x):
        ...
        for q in range(R):  # run over episodes
            for p in range(N-1):  # run over number of collocation nodes
        ...
        for i in range(0, R):
            for j in range(0, N-1):
                ...
                y = np.array([i*N*S+j*S, i*N*S+j*S+2, i*N*S+j*S+4, i*N*S+j*S+6, 
                          ...
                          i*N*S+j*S+5, i*N*S+j*S+6, i*N*S+j*S+7, R*N*S, R*N*S+1, R*N*S+4,
                          
                          i*N*S+j*S, i*N*S+j*S+1, i*N*S+j*S+2, i*N*S+j*S+3, i*N*S+j*S+4,
                          i*N*S+j*S+5, i*N*S+j*S+6, i*N*S+j*S+7, R*N*S+2, R*N*S+3, R*N*S+5
                          ])
    ...
    def dynamic_fun(self, x, xdot, p, a, noise):
        ...
        f[2] = (a*d_L*m_L*cos(theta_a) + a*d_T*m_T*cos(theta_a + theta_h) 
                ...
                - (k00*(theta_a-ref_a) + k01*omega_a) - noise[0])
        ...
        dfdx[2,2] = -2*d_T*l_L*m_T*omega_a*sin(theta_h) + d_T*l_L*m_T*(2*omega_a + 2*omega_h)*sin(theta_h) - k01
            
        dfdx[2,3] = d_T*l_L*m_T*(2*omega_a + 2*omega_h)*sin(theta_h)
        ...
        f[3] =  (a*d_T*m_T*cos(theta_a + theta_h) + d_T*g*m_T*sin(theta_a + theta_h) 
                ...
                - (k10*(theta_h-ref_h) + k11*omega_h) - noise[1])

            
        dfdx[3,0] = - a*d_T*m_T*sin(theta_a + theta_h) + d_T*g*m_T*cos(theta_a + theta_h)
            
        dfdx[3,1] = (-a*d_T*m_T*sin(theta_a + theta_h) + d_T*g*m_T*cos(theta_a + theta_h) 
                    - d_T*l_L*m_T*omega_a**2*cos(theta_h) + d_T*l_L*m_T*omega_a_dot*sin(theta_h) - k10)
