...
def ITP(Y, con, time):
    ...
    ForceVec = np.matrix([[omega_a], [omega_h], [(-kp03*omega_h + a*d_L*m_L*cos(theta_a) + a*d_T*m_T*cos(theta_a + theta_h) 
            ...
            + g*l_L*m_T*sin(theta_a) - kp00*(theta_a-ref_a) - kp01*(theta_h-ref_h) - kp02*omega_a)],
    
                [(a*d_T*m_T*cos(theta_a + theta_h) + d_T*g*m_T*sin(theta_a + theta_h) 
            - d_T*l_L*m_T*omega_a**2*sin(theta_h) 
            - (kp10*(theta_a-ref_a) + kp11*(theta_h-ref_h) + kp12*omega_a + kp13*omega_h))]])
...
for eps in range(num_eps):
    ...
    folder = 'FPD/Analysis/Eps'+str(eps_r)
