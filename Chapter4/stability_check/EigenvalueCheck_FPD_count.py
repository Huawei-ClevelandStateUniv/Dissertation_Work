...
def controller(x, xdel, p):
    ...
    Tor[0] = Kpaa*(ra-x[0]) + Kdaa*(-x[2]) + Kpah*(rh-x[1]) + Kdah*(-x[3])
    Tor[1] = Kpha*(ra-x[0]) + Kdha*(-x[2]) + Kphh*(rh-x[1]) + Kdhh*(-x[3])
...
num_par = 10  # number of the control parameters
...
folder = 'FPD/Analysis/Eps'+str(eps)+'/'      
