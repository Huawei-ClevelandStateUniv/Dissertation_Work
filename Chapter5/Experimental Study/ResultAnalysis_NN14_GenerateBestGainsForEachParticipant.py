...
num_par = (num_states+1)*hidden_nodes + (hidden_nodes+1)*2 


for subj in range(3, 9):
    for trial in range(1, 3):

        store_path = 'NN14/Analysis_Paper/Subj0'+str(subj)+'Pert'+str(trial)+'_GFL_NN14/'
        if not os.path.exists(store_path):
            os.makedirs(store_path)
            
        data_path = 'NN14/Results/Subj0'+str(subj)+'/Pert'+str(trial)+'_GFL_NN14_Eps1_Noise025_SameInit/'
        ...
        for k in range(St, Ed):
            opt_sta[k, :] = np.loadtxt(data_path + 'RMS_Sta_'+str(trial-1)+str(k)+'.txt')[:2]
            opt_con[k, :] = np.loadtxt(data_path + 'Controller_'+str(trial-1)+str(k)+'.txt')
