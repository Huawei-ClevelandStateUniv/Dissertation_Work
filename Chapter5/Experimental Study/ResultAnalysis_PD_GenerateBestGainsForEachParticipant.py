...
#        Trail = 1
        ...
        num_par = num_states + delay_nodes*num_states*2 + 2
        St = 0
        En = 10
        ...
        StorePath = 'PD/Analysis_Paper/Subj0'+str(Subj)+'/Pert'+str(Trail)+'_GFL_SPDR_Eps2_Noise025/'
        if not os.path.exists(StorePath):
            os.makedirs(StorePath)
        
        
        folder = 'PD/Result/Subj0'+str(Subj)+'/Pert'+str(Trail)+'_GFL_SPDR_Eps2_Noise025/'
        ...
        plt.xticks(ind1, ('Kp_a', 'Kd_a'))
        ax2 = fig1.add_subplot(3,1,2)
        plt.ylabel('N*m/rad (N*m*s/rad)')
        plt.xticks(ind1, ('Kp_h', 'Kd_h'))
        ax3 = fig1.add_subplot(3,1,3)
        plt.ylabel('Degree')
        plt.xticks(ind1, ('Ref_a', 'Ref_h'))

        ax1.bar(ind1, ConAve[:2], width, yerr=ConStd[:2], error_kw=dict(elinewidth=4, ecolor='red'))
        ax2.bar(ind1, ConAve[2:4], width, yerr=ConStd[2:4], error_kw=dict(elinewidth=4, ecolor='red'))
        ax3.bar(ind1, ConAve[4:6]*180/np.pi, width, yerr=ConStd[4:6]*180/np.pi, error_kw=dict(elinewidth=4, ecolor='red'))
