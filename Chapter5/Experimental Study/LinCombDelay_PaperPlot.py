...
for sj in range(num_subj):
    ...
    for tr in range(2):
        ...
        store_path = 'LinCombDelay/Analysis_Paper/Subj0'+str(subj)+'Pert'+str(tr+1)+'_GFL_LinComb_delay60/'
        
        Fit_Info[sj, tr, :] = np.loadtxt(store_path + 'Best_RMS_R.txt')
        
        
with open('LinCombDelay/Analysis_Paper/Lowest_RMS.txt', 'w') as outfile:
    StringP = ''
...
with open('LinCombDelay/Analysis_Paper/Highest_R1.txt', 'w') as outfile:
    StringP = ''
    for i in range(num_subj):
        for j in range(2):
...
with open('LinCombDelay/Analysis_Paper/Highest_R2.txt', 'w') as outfile:
    StringP = ''
    for i in range(num_subj):
        for j in range(2):
