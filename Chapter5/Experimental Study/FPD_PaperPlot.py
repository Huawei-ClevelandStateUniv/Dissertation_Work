...
for sj in range(num_subj):
    ...
    for tr in range(2):
        store_path = 'FPD/Analysis_Paper/Subj0'+str(subj)+'/Pert'+str(1+tr)+'_GFL_FPD_Eps3_Noise025/'
        
        Mean_Gains[sj, tr, :] = np.loadtxt(store_path + 'Best_Con.txt')
        sort_con = np.loadtxt(store_path + 'Sort_Con.txt')
        if len(sort_con.shape) == 1:
            Best_Gains[sj, tr, :] = sort_con
        else:
            Best_Gains[sj, tr, :] = sort_con[0, :]
        ...
        Mean_Gains[sj, tr, [0, 2, 4, 6]] = Mean_Gains[sj, tr, [0, 2, 4, 6]]/Kp_normalization
        Mean_Gains[sj, tr, [1, 3, 5, 7]] = Mean_Gains[sj, tr, [1, 3, 5, 7]]/Kd_normalization
        
        Best_Gains[sj, tr, [0, 2, 4, 6]] = Best_Gains[sj, tr, [0, 2, 4, 6]]/Kp_normalization
        Best_Gains[sj, tr, [1, 3, 5, 7]] = Best_Gains[sj, tr, [1, 3, 5, 7]]/Kd_normalization
        
        STD_Gains[sj, tr, [0, 2, 4, 6]] = STD_Gains[sj, tr, [0, 2, 4, 6]]/Kp_normalization
        STD_Gains[sj, tr, [1, 3, 5, 7]] = STD_Gains[sj, tr, [1, 3, 5, 7]]/Kd_normalization
        
with open('FPD/Analysis_Paper/Lowest_RMS.txt', 'w') as outfile:
    StringP = ''
...
with open('FPD/Analysis_Paper/Highest_R1.txt', 'w') as outfile:
    StringP = ''
    for i in range(num_subj):
        for j in range(2):
...
with open('FPD/Analysis_Paper/Highest_R2.txt', 'w') as outfile:
    StringP = ''
    for i in range(num_subj):
        for j in range(2):
...
ind_ankle = np.array([0, 2, 1, 3])
ind_hip = np.array([5, 7, 4, 6])
ind_td_ref = np.array([8, 9])
...
ax1.set_xticklabels(['K11', 'B11', 'K21', 'B21'])
ax1.set_xticks(list(ind1))

ax2.set_xlim([-1, len(ind1)])
ax2.set_xticklabels(['K22', 'B22', 'K12', 'B12'])
