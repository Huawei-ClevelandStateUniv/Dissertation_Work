function [figurehandle, b] = errorfill(x, y, varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	% Check arguments.
	if(ischar(varargin{end-1}))										% Get LineSpec.
		lSpec=		[lSpec varargin{end-1}];
		N=			N-1;
    end
    
    if (N == 2), transp = varargin{end}; N = N-1;              % get transparency
    elseif (N == 1), transp = 1; end                                    % set default transparency as 1
	...
	for n= 1:N
		a = patch(x(1:2*len), E(:,n), colorshade(1,n,:), 'LineStyle', 'none');	% Draw error fill.
        a.FaceAlpha = transp;                                                   % set transpancy
	end
	b = plot(x(1:len), y, lSpec, 'Color', color, 'linewidth', 2);		% Draw y(x).
	...
	b.Color(4) = transp;                                           % set transpancy
	% Notify.
